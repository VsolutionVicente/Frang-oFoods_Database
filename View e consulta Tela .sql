...
SELECT ID_MULTPROGPROD, 
       COALESCE(ID_ETIQUETA, 0) ID_ETIQUETA,
       QN_CAIXMULTPROGPROD,
       GN_TAGMULTPROGPROD
  FROM VW_PRODUCAO_PROG_MULT_TOTA
 WHERE EMPRESA = :EMPRESA
   AND FILIAL = :FILIAL
   AND ID_ESTRUNIDABAT = :ID_ESTRUNIDABAT
   AND DT_MULTPROGPROD = :DT_MULTPROGPROD
   AND ID_PRODMATEEMBA = :ID_PRODMATEEMBA
   AND NR_SEQUTURNMULTPROGPROD = :NR_SEQUTURNMULTPROGPROD
   AND QN_CAIXMULTPROGPROD > QN_CAIXATENMULTPROGPROD
   AND (COALESCE(ID_ETIQUETA, 0) > 0 OR COALESCE(GN_TAGMULTPROGPROD, '') <> '')
 ORDER BY NR_PRIOMULTPROGPROD    